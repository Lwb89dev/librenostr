...
class ScanCodeViewModel @Inject constructor(
    ...
    private fun processCodeForAnything(code: String) {
        val type = QrCodeDataType.from(code)
        when (type) {
            ...
            // without a wallet of our own, so they fall through to the "not recognized" state.
            QrCodeDataType.LNBC -> setEffect(SideEffect.PayLightningInvoice(invoice = code))

            QrCodeDataType.NOSTR_CONNECT -> {
                setEffect(SideEffect.NostrConnectRequest(url = code))
            }

            null, QrCodeDataType.LNURL, QrCodeDataType.LIGHTNING_URI,
